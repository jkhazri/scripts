...
def generate_files(access_method, ip_addresses, usernames, ssh_password_storage_path, infra_groupname, ssh_key_storage_path, node_names, node_ids):
    if not (len(ip_addresses) == len(usernames) == len(node_names) == len(node_ids)):
        print("ERROR: The number of IP addresses, usernames, node names, and node IDs must be the same.")
        return
...
[{infra_groupname}]
"""
        for ip, user, node, node_id in zip(ip_addresses, usernames, node_names, node_ids):
            hosts_content += f"{node} ansible_host={ip} ansible_user={user} node_id={node_id} impacted_server_ip={ip}\n"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--node_ids", type=str, required=True, help="Comma-separated list of node IDs")
